...
- (id)initWithImage:(UIImage*)newImage andframeSize:(CGSize)frameSize andcropSize:(CGSize)cropSize
{
    ...
    if(self)
    {
        ...
        _imageScale = WIDTH/cropSize.width ;
        
        imgViewBound = CGRectMake(0, 0, self.inputImage.size.width*_imageScale, self.inputImage.size.height*_imageScale);   //이미지가 생성될 사이즈.
        ...
        _cropperView.frame = CGRectInset(_cropperView.frame, -borderWidth, -borderWidth);
        _cropperView.layer.borderColor = [UIColor whiteColor].CGColor;
        _cropperView.layer.borderWidth = borderWidth;
        
        //        CALayer *borderLayer = [CALayer layer];
        //        CGRect borderFrame = CGRectMake(0, -1, (_cropperView.frame.size.width), (_cropperView.frame.size.height+2));
        //        [borderLayer setBackgroundColor:[[UIColor clearColor] CGColor]];
        //        [borderLayer setFrame:borderFrame];
        //        [borderLayer setCornerRadius:kCornerRadius];
        //        [borderLayer setBorderWidth:1.0];
        //        [borderLayer setBorderColor:[[UIColor whiteColor] CGColor]];
        //        [_cropperView.layer addSublayer:borderLayer];
        
        _cropperView.alpha = 1.0;
        
        hei = _cropRect.origin.y;
...
- (UIImage *)imageNamed:(UIImage *) baseImage withTintColor: (UIColor *) tintColor {
...
{
    _inputImage = setImage;
    _imgView.image = setImage;
}
...
- (void)zoomAction:(UIGestureRecognizer *)sender
{
    ...
    if ([sender state] == UIGestureRecognizerStateChanged || [sender state] == UIGestureRecognizerStateEnded)
    {
        ...
        //NSLog(@"X:%f, Y:%f, WIDTH:%f, HEIGHT:%f",imgViewFrame.origin.x,imgViewFrame.origin.y,imgViewFrame.size.width,imgViewFrame.size.height);
        //역변환 공식
        ...
        if(collideState >0)  // 걸렸을 때
        {
            ...
            else
            {
                ...
                [UIView animateWithDuration:0.5f animations:^(void) {
                    ...
                    /*
                    if (imgframe.size.width > imgframe.size.height)
                    {
                        self.imgView.frame = imgframe;
                    }
                    else
                    {
                        self.imgView.frame = _cropperView.frame;
                    }
                     */
...
- (UIImage*)getCroppedImage
{
    ...
    CGPoint cropperViewOrigin = CGPointMake((_cropperView.frame.origin.x - self.imgView.frame.origin.x)  *1/zoomScale ,
                                            (_cropperView.frame.origin.y - self.imgView.frame.origin.y) * 1/zoomScale
                                            );
    ...
    UIImage *rotInputImage = [[_imgView.image fixOrientation] imageRotatedByRadians:rotationZ];
