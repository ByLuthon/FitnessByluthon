...
- (void)deleteImageWithUrl:(NSURL *)assetUrl
                    ...
                    if(asset.isEditable)
                    {
                        [asset setImageData:nil metadata:nil completionBlock:^(NSURL *assetURL, NSError *error) {
                            NSLog(@"Asset url %@ should be deleted. (Error %@)", assetURL, error);
                                ...
                                NSString *appAlbumName = [NSString stringWithFormat:@"%@",APP_Photos_AlbumName];
            
                                if (IS_SAMESTRING(thisGroup, appAlbumName))
                                {
                                    [group enumerateAssetsUsingBlock:^(ALAsset *asset, NSUInteger index, BOOL *stop) {
                                        if(asset.isEditable && IS_SAMESTRING([asset.defaultRepresentation.url absoluteString], [assetUrl absoluteString])) {
...
- (void)loadImagesFromAlbum:(NSString *)albumName completion:(void (^)(NSMutableArray *, NSError *))completion
{
  ALAssetsLibraryGroupsEnumerationResultsBlock block = ^(ALAssetsGroup *group, BOOL *stop) {
    ...
    if ([albumName compare:[group valueForProperty:ALAssetsGroupPropertyName]] == NSOrderedSame) {
      ...
      [group enumerateAssetsUsingBlock:^(ALAsset *result, NSUInteger index, BOOL *stop)
       ...
       // UIImageOrientation orientation = (UIImageOrientation)[[result valueForProperty:@"ALAssetPropertyOrientation"] intValue];
       // UIImage * image = [UIImage imageWithCGImage:[[result defaultRepresentation] fullScreenImage]scale:1.0 orientation:orientation];
