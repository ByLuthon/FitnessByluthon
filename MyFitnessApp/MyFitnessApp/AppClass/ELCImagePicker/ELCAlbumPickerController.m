...
- (void)viewDidLoad
{
    ...
    [self.navigationItem setTitle:NSLocalizedString(@"str_txtLoadingLoader", nil)];
    
    UIBarButtonItem *cancelButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCancel target:self.parent action:@selector(cancelImagePicker)];
    [self.navigationItem setLeftBarButtonItem:cancelButton];
    cancelButton.tintColor = [UIColor purpleColor];
    
    
    UIButton* button = [UIButton buttonWithType:UIButtonTypeCustom];
    [button setImage:[UIImage imageNamed:@"cam.png"] forState:UIControlStateNormal];
    ...
    dispatch_async(dispatch_get_main_queue(), ^
                   {
                       @autoreleasepool {
                           ...
                           void (^assetGroupEnumerator)(ALAssetsGroup *, BOOL *) = ^(ALAssetsGroup *group, BOOL *stop)
                           {
                               ...
                               /*
                               NSString *strCamera;
                               if (APPDELEGATE.selectedLanguage == CHINESE_LANGUAGE)
                               {
                                   strCamera = @"相机胶卷";
                               }
                               else
                               {
                                   strCamera = @"camera roll";
                               }
                                */
                           ...
                           //NSUInteger groupTypes = ALAssetsGroupAlbum | ALAssetsGroupEvent | ALAssetsGroupFaces | ALAssetsGroupSavedPhotos | ALAssetsGroupAll;
...
{
    [self.tableView reloadData];
    [self.navigationItem setTitle:NSLocalizedString(@"Album", nil)];
}
...
- (void)cameraAction:(id)sender
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"openCameraFromELCPicker" object:nil userInfo:nil];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //cell.textLabel.font = setDefaultFont(14);
    
    UIImage* image = [UIImage imageWithCGImage:[g posterImage]];
    image = [self resize:image to:CGSizeMake(65, 65)];
