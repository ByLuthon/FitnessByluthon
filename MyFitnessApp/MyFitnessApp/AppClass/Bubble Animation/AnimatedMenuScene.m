...
- (void)updateScene
{
    ...
    for (NSString *nodeTitle in _menuNodes)
    {
        ...
        [menuItem setFillColor:RGBA(56, 46, 237, 1)];
        [self addChild:menuItem];
    }
}
...
- (void)selectNode:(SKNode*)node
{
    ...
    if (_allowMultipleSelection)
    {
        // This is for add selection image
        /*
         SKSpriteNode* node1 = [[SKSpriteNode alloc] initWithImageNamed:@"nodeImg.png"];
         node1.name = @"workoutSelection";
         node1.size = CGSizeMake(30, 30);
...
-(void)handleDoubleTap :(SKNode *)node
{
    NSLog(@"tapCount 2");
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    if (!_moving)
    {
        ...
        if(numTaps == 2)
        {
            ...
            [self performSelector:@selector(handleDoubleTap:) withObject:node afterDelay:delay];
        }
    }
    _moving = NO;
}
